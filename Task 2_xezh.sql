...
CREATE table employees(EMPLOYEE_ID int primary key, NAME text NOT NULL, DEPARTMENT text, EMAIL text, PHONE_NO numeric(15),JOINING_DATE date, 
SALARY numeric(12,2), BONUS numeric(12,2), TAX_PERCENTAGE numeric(5,2));
...
LIMIT 1;
# method 2 CTE in case there is same rank
with salary as(
...
denserank as(
select DEPARTMENT, avg_salary,dense_rank() over (order by (avg_salary) desc) as rnk
from salary)
select DEPARTMENT,avg_salary,rnk
from denserank
where rnk=1;
